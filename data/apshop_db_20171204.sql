...
systemCName		VARCHAR(100) NOT NULL			COMMENT '子系统的中文名',
...
configDESC			TEXT NOT NULL					COMMENT '配置参数说明',
...
docUrl				TEXT							COMMENT '文档的Url链接',
docType			    CHAR(1)	DEFAULT 'D' NOT NULL	COMMENT '文档类型(*,D=文档,P=图片,M=视频,O=其他文件)',
...
-- 其他信息
emall				VARCHAR(100)					COMMENT '邮箱(登录识别)',
cell2				VARCHAR(100)        			COMMENT '备用手机(可录入多个)',
...
graduationDate		DATE							COMMENT '毕业时间',
...
URGLinkman			VARCHAR(30)						COMMENT '紧急联系人',
URGTel				VARCHAR(30)						COMMENT '紧急联系人电话',
...
CREATE TABLE `shop_goodsClass` (
    `goodsClassID`     varchar(45) NOT NULL                    COMMENT 'ID',
    `parentId`         varchar(45)  NOT NULL                   COMMENT '父级ID',
    `name`             varchar(100)                             COMMENT '名称',
    `isLeaf`            CHAR(1)                                 COMMENT '是否为叶子节点',
    sortNo				INT DEFAULT 0 NOT NULL			        COMMENT '显示顺序',
...
CREATE TABLE `shop_goods` (
  ...
  `goodsType`            CHAR(1) DEFAULT 'T' NOT NULL	    COMMENT '商品类型(T=主题, C=组件)',
